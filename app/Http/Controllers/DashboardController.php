...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $dashboardStats = [
            ...
            'completed_courses_count' => UserCourseProgress::where('user_id', $currentUser->user_id)
                                    ->where('done', 1)
                                    ->count(),
            'watch_later_count' => WatchLater::where('user_id', $currentUser->user_id)->count(),
            'total_learning_hours' => 24
        ];

        $featuredCoursesList = Course::with('roadmaps')
            ->take(6)
            ->get();

        $popularRoadmapsList = Roadmap::withCount(['courses', 'userRoadmaps'])
            ->orderByDesc('user_roadmaps_count')
            ->take(3)
            ->get();
        ...
        $recentUserActivity = UserCourseHistory::query()
            ...
            ->take(4)
            ->get([
                'user_course_history.last_seen',
                'detail_course.name as detail_name',
        ...
        $watchLaterCourses = WatchLater::query()
            ->join('course', 'watchlater.course_id', '=', 'course.course_id')
            ...
            ->get([
                'course.course_id',
                'course.name as course_name',
                'course.desc as course_description'
            ]);
