                ...
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            {{-- Hanya tampilkan tombol aksi (Bookmark & Done) jika ada materi ($current) yang sedang aktif --}}
                            @if ($current)

                            {{-- Tombol Bookmark --}}
                            @if ($isBookmarked)
                            {{-- Form untuk Hapus (Remove) --}}
                            <form action="{{ route('watchlater.destroy', $watchLaterItem->watchlater_id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-primary btn-md" title="Remove from Watch Later">
                                    {{-- Ikon solid (sudah di-bookmark) --}}
                                    <i class="fa-solid fa-bookmark"></i>
                                </button>
                            </form>
                            @else
                            {{-- Form untuk Tambah (Store) --}}
                            <form action="{{ route('watchlater.store') }}" method="POST" style="display:inline;">
                                @csrf
                                <input type="hidden" name="detail_course_id" value="{{ $currentDetail->detail_course_id }}">
                                <button type="submit" class="btn btn-outline-primary btn-md" title="Add to Watch Later">
                                    {{-- Ikon reguler (belum di-bookmark) --}}
